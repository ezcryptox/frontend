			...
			<nav>
				{#if selectedTab == 0}
					...
					{:else}
						{#each userNotifications as activity}
							<li>
								<div class="a7c0fc49" aria-read-status={activity.readStatus}>
									<div class="eb25c4fd">
										<div class="b75ca08f">
											<img src={activity.icon} alt="360002708233" /><span>{activity.category}</span>
										</div>
										<span
						...
						{/each}
						{#if !isLoadingUN && !platformNotifications?.length}
							<Empty />
						{/if}
					{/if}
				{:else if selectedTab === 1}
					...
					{:else}
						...
						{/each}
						{#if !isLoadingPN && !platformNotifications?.length}
							<Empty />
						{/if}
					{/if}
				{/if}
