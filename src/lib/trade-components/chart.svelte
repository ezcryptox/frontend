<script>
	import { currentSelectedPair, tradePairs, tradeTabs, cryptoQuotes  } from '$lib/store/marketdata';
	...
	const onCloseTab = (asset) => {
		let tabs = $tradeTabs.filter((_asset) => _asset.symbol !== asset.symbol);
	...
	onMount(() => {
		...
		currentSelectedPair.subscribe(async (asset) => {
			if (asset && asset.symbol !== prevSelected) {
				await tick()
				if (!$tradeTabs.find((tab) => tab.symbol === asset.symbol)) {
					const _tabs = $tradeTabs;
						if (_tabs.length >= 5) {
						_tabs.shift();
					}
				_tabs.push({
				...
				if (tvWidget) {
					...
					appendScript(() => {
						initWidget({
							...
							datafeed:
								Datafeed /*new Datafeeds.UDFCompatibleDatafeed(`${ServerURl()}/api/market`, 10, {
			...
			expectedOrder: 'latestFirst'
							...
							disabled_features: ["header_widget", "timeframes_toolbar", "volume_force_overlay", "use_localstorage_for_settings"],
              enabled_features: ["move_logo_to_main_pane", "side_toolbar_in_fullscreen_mode",'study_templates'],
							// disabled_features: ['header_widget', 'timeframes_toolbar', 'symbol_info'],
		...
		<section class="_7f9800af">
			<div class="f2ad5021 _8eddb339">
				<div class="ea55d85f">
					...
					<div class="_5daf8810">
						<span class="_9391ea7a">{$_('g-date')}</span>
						{#each intervalFavList as interval}
							<!-- svelte-ignore a11y-no-static-element-interactions -->
							<!-- svelte-ignore a11y-click-events-have-key-events -->
							<span
								on:click={handleIntervalChange(interval)}
								class="_9391ea7a{interval === activeInterval ? ' d6ac01f2' : ''}">{interval}</span
							>
						{/each}
						<!-- svelte-ignore a11y-no-static-element-interactions -->
						<span
						...
						>
							<span class="_432cfdda{!intervalFavList.includes(activeInterval) ? ' d6ac01f2' : ''}"
								>{!intervalFavList.includes(activeInterval) ? activeInterval : $_('g-more')}</span
							>
							<div class="_41c42367">
									{#each supportedIntervals as interval}
										...
										<span on:click={handleIntervalChange(interval)} class="_3b5033be">
											...
											<span class="a53c3f73{interval === activeInterval ? ' d6ac01f2' : ''}"
												>{interval}</span
											><span>
												<!-- svelte-ignore a11y-no-static-element-interactions -->
												<!-- svelte-ignore a11y-click-events-have-key-events -->
				...
				<div class="d66c85b6">
					...
					<span
						...
						class={currChartType === 'time' ? '_63bc7ce4' : ''}>{$_('tradingview')}</span
					>
					<span class="_0fc78741"></span>
					<!-- svelte-ignore a11y-click-events-have-key-events -->
					<!-- svelte-ignore a11y-no-static-element-interactions -->
					<span
						on:click={handleChartType('depth')}
						class={currChartType === 'depth' ? '_63bc7ce4' : '_0fc78741'}>{$_('depth')}</span
					><span class="_0fc78741"></span>
					<!-- svelte-ignore a11y-click-events-have-key-events -->
					<!-- svelte-ignore a11y-no-static-element-interactions -->
