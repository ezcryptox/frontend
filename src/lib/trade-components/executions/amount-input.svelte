...
<div data-key="amount" class="form-item f3a05fb6" style="{style}" data-v-39752d79="">
	<!---->
	<div class="form-item-content">
		...
		<span class="input-prefix">{placeholder}</span>
			...
			{#if dropdownOption.items.length > 0}
				...
				<div bind:this={divRef} class="c3d3da83" 
				on:mouseover={(e) => {
				...
				>
					{#each dropdownOption.items as item, index}
						...
						<li on:click={() => {
							...
							showDropdownMenu = false; 
						}} tabindex="-1" class="e45ef080 {dropdownOption.selectedIndex === index ? '_1007348c' : ''}">{item}</li>
					{/each}
				</ul>
