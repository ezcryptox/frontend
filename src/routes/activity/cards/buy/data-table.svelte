<script lang="ts">
	...
	import { selectedFilter, orders } from './store';
	import { onMount } from 'svelte';
	import * as Table from '$lib/components/ui/table';
	let loadingOrders = false;
	let table: any = null;
	async function loadOrders(_selectedFilter) {
		loadingOrders = true;
		try {
			let filters = {
				start: _selectedFilter.time.start,
				end: _selectedFilter.time.end
			};
			if (_selectedFilter.fiat !== 'All') {
				filters['fiat'] = _selectedFilter.fiat;
			}
			if (_selectedFilter.crypto !== 'All') {
				filters['crypto'] = _selectedFilter.crypto;
			}
			if (_selectedFilter.status !== 'All') {
				filters['status'] = _selectedFilter.status;
			}
			const { list } = (
				await axios.get(
					`${ServerURl()}/api/transactions/list-orders?limit=100&${Object.keys(filters)
	...
	selectedFilter.subscribe(filters => {
		loadOrders(filters);
	});
</script>
