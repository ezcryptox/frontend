<script lang="ts">
	...
	$: fetchOrder =
		...
		async () => {
			return axios
				.get(
					`${ServerURl()}/api/assets/borrowed?${filters.coin !== 'All' ? `coin=${filters.coin}&` : ''}start=${filters.time.start}&end=${filters.time.end}`,
					{
						headers: {
							...
							<div class="ag-theme-alpine po-ag-table">
								<DataListTable
									dataListColumns={[
										...
										{ accessor: 'amount', header: 'Debt Amount' },
										{ accessor: 'interest', header: 'Interest' },
										{ accessor: 'interest_rate', header: 'Hourly Interest Rate' },
									]}
