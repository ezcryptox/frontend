<script lang="ts">
	...
	async function fetcher() {
		return Object.keys(vipLevels).map((k) => ({
			level: k,
			spot: `$${vipLevels[k].spotTradingVolume.toLocaleString()}`,
			futures: `$${vipLevels[k].futuresTradingVolume.toLocaleString()}`,
			balance: `$${vipLevels[k].totalBalance.toLocaleString()}`,
			asks_bids: `${(vipLevels[k].makerTaker.maker * 100).toFixed(4)}%/${(vipLevels[k].makerTaker.taker * 100).toFixed(4)}%`
		}));
	}
</script>
				...
				<div class="J-vm8">
					...
					<div class="QWTt5">
								...
								<ul>
									<li class="">
										<a
											aria-current="page"
											href="/fee/spot"
											class="miAZa router-link-exact-active yXhKP">Spot</a
										>
									</li>
									<!-- <li class="">
						...
						<DataListTable
							...
							tableRowClasses={(row) => {
								if (!!data && row.cells[0].value === `VIP${data?.level - 1}`)
									return 'border-b-[#e59940]';
